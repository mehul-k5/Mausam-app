...
public final class NetworkUtils {
     ...
     * 2) Later in Mausam, you are going to add an alternate method of allowing the user
    ...
    public static URL getUrl(Context context) {
        if (MausamPreferences.isLocationLatLonAvailable(context)) {
            double[] preferredCoordinates = MausamPreferences.getLocationCoordinates(context);
            double latitude = preferredCoordinates[0];
            double longitude = preferredCoordinates[1];
            return buildUrlWithLatitudeLongitude(latitude, longitude);
        } else {
            String locationQuery = MausamPreferences.getPreferredWeatherLocation(context);
    ...
    private static URL buildUrlWithLatitudeLongitude(Double latitude, Double longitude) {
        ...
        try {
            URL weatherQueryUrl = new URL(weatherQueryUri.toString().concat("&appid="+ BuildConfig.OPEN_WEATHER_MAP_API_KEY));
    ...
    private static URL buildUrlWithLocationQuery(String locationQuery) {
        ...
        try {
            URL weatherQueryUrl = new URL(weatherQueryUri.toString().concat("&appid="+ BuildConfig.OPEN_WEATHER_MAP_API_KEY));
            Log.v(TAG, "URL_location: " + weatherQueryUrl);
            return weatherQueryUrl;
        } catch (MalformedURLException e) {
            e.printStackTrace();
            return null;
